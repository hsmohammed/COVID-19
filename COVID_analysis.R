...
COVID_confirmed <- COVID_confirmed %>% 
  pivot_longer(cols      = c('1/22/20':names(COVID_confirmed)[ncol(COVID_confirmed)]),
               names_to  = "date",
               values_to = "n_cases")

COVID_deaths <- COVID_deaths %>% 
  pivot_longer(cols      = c('1/22/20':names(COVID_deaths)[ncol(COVID_deaths)]),
               names_to  = "date",
               values_to = "n_cases")

COVID_recovered <- COVID_recovered %>% 
  pivot_longer(cols      = c('1/22/20':names(COVID_recovered)[ncol(COVID_recovered)]),
               names_to  = "date",
               values_to = "n_cases")
...
plot_country <- function(country) {
  ...
  COVID_confirmed_country <- xts(x = COVID_confirmed$country, 
                                 order.by = as.Date(COVID_confirmed$date,format = '%m/%d/%Y'))
  ggplot()+
    geom_line(data = fortify(COVID_confirmed_country),aes(Index,COVID_confirmed_country), color = "blue")+
    theme_bw()
...
date_vec <- as.Date(COVID_confirmed$date,format = '%m/%d/%Y')
...
plot_country <- function(df, country, type){
  ...
  p1 <- ggplot()+
    geom_line(data = df,aes(date,df[[country]], color = "blue"))+
     theme_bw()+
...
plot_country(COVID_confirmed, "Egypt", "Confirmed")
